...
const isAntiVO = isGroup ? antiviewonce.includes(from) : false	
		...
		const katalog = (teks) => {
             res = pemuda.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "© Fahri", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             pemuda.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = pemuda.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://tch/tchh', "groupName": `${NamaBot}`, "footerText": "© Fahri", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
	...
	//•======>> SUBSCRIBE : PEMUDA TEKNOLOGI <<====•\\
        ...
        const sendWebp = async(from, url) => {
	        ...
	        pemuda.sendMessage(from, { disname: nama, vcard: vcard}, MessageType.contact, {contextInfo: {"mentionedJid": ane}})
				...
				const reply = (teks) => {
			pemuda.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title:`Hai ${pushname}`,body:"",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/6283163410880?text=Assalamualaikum%20Kakak%20Chandra%20yang%20Ganteng`}}})
		}
		const simir = (teks) => {
...
case '🕔sewa':
       ...
       menu = `0`
sendButLocation(from, `_*Hai ${pushname}🌹*_\n\nSilahkan pilih metode pembayaran`, "*Project Pemuda © 2022*", {jpegThumbnail:ofrply}, [{buttonId:`${prefix}py`,buttonText:{displayText:'NOMOR 📱'},type:1},{buttonId:`${prefix}qrsewa`,buttonText:{displayText:'QR CODE 📱'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
break
case 'py':
        case '0':
        stod = `${sender}`
...
listMsg = {
 buttonText: 'NOMOR',
 footerText: '*Pemuda Teknologi © 2022*',
...
const kontil = body.slice(8)
                    ...
                    pemuda.sendMessage('6283168004413@s.whatsapp.net', options, text, {quoted: mek})
